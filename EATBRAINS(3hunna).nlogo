...
humans-own [ stamina role builder hunter doctor nextcolor pleb infected? safecolor ];notded?
...
to hunter-behavior
  if one-of zombies in-radius hunter-kill-radius
    [die]
end
...
[die]
; Moving Around
  if[pcolor] of patch-here = red or [pcolor] of patch-here = white
     [shambleback]
  ifelse any? humans with [infected? != true] in-cone zombie-vision-radius zombie-vision-angle
       [
         face min-one-of humans [distance myself] fd zombie-speed + 0.1
       ]
       [shamble]
    ask zombies
  [ ifelse rotting != 0
...
to targetdoctors
  ...
  [
    face min-one-of humans with [role = doctor] [distance myself]
    fd zombie-speed * 1.5
  ]
end
to shamble
