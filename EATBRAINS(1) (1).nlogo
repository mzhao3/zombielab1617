...
humans-own [ stamina builder hunter doctor nextcolor pleb infected? ]
globals [ cure-development symptoms-show infectionchance ]
zombies-own [ rottingrate patient0? ]
...
;;;
  create-humans Starting-Humans
   [
    ...
    if pxcor >= 30 [set xcor xcor - random 30 - 20]
    set size 1
  ]
end
...
;;;
to human-behavior
    ifelse any? (zombies with [patient0? != true] ) in-radius human-vision-radius
    [
...
to zombie-behavior
  ...
  ;; flocking // zombie horde // hive mind
end
...
to rot
  ifelse rottingrate - rotting-rate > 0
  [
    set rottingrate (rottingrate - rotting-rate)
  ]
  [ crumble ]
end
to crumble
