...
humans-own [ stamina role builder hunter doctor nextcolor pleb infected? safecolor]
...
to doctor-behavior
  ask humans with [role = doctor]
    [set cure-development (cure-development + (cure-development-rate / 1000) * count (humans with [role = doctor])) ]
end
...
[die]
  ...
  ifelse any? humans with [infected? != true] in-cone zombie-vision-radius zombie-vision-angle
    ...
    ask zombies
  [ ifelse rotting != 0
    [ set rotting (rotting - rotting-rate / 100) ]
    [crumble]
  ]
...
to rot
  ifelse rotting - rotting-rate > 0
  [
    set rotting (rotting - rotting-rate)
  ]
  [ crumble ]
end
to crumble
...
to go
 ...
 ;   ask patches [set pcolor orig-pcolor ]
      ...
      ask patient0s [paitent0-behavior]
      ; animate
      tick
    ]
end
