...
humans-own [ stamina role builder hunter doctor nextcolor pleb infected? safecolor infectionlength];notded?
globals [ cure-development symptoms-show infectionchance  ]
...
;;;
ask patches with
[0 = pxcor mod 8 and 0 = pycor mod 8 and pxcor < 29]
[set pcolor gray]
  repeat 2
  [
    ask patches with
...
to hunter-behavior
  ask humans with [role = hunter and role != doctor and role != builder and role != pleb]
  [
    if any? zombies
    [
...
to doctor-behavior
  ask humans with [role = doctor and role != hunter and role != builder and role != pleb]
  [
...
to developcure
    set cure-development (cure-development + (cure-development-rate / 100) ) * count (humans with [role = doctor])
    ; Maggie 1/11/2017
end
to cure
  ask zombies-here [
...
to builder-behavior
  ask humans with [role = builder and role != doctor and role != hunter and role != pleb]
  [
  fixsh*t
  ]
...
;; moving around
  ifelse
  [pcolor] of patch-here = red or
  [pcolor] of patch-here >= white - 5
  [shambleback]
  [
...
to breaksh*t
  ask zombies [
   ; if [pcolor] of patch-ahead 1 = white
     ; [set ([pcolor] of patch-ahead 1) ([pcolor] of patch-ahead 1) - 1]
...
to killdoctors
  if any? humans with[role = doctor] in-cone zombie-vision-radius zombie-vision-angle
  [
      face min-one-of humans with [role = doctor] [distance myself] fd zombie-speed * 1.5
    ]
  end
...
; paitent zer0 behavior
;;;
;;;
to paitent0-behavior
  ask patient0s
[
...
to go
    every 1 / 30
  [
 ;   ask patches [set pcolor orig-pcolor ]
      human-behavior
      hunter-behavior
      doctor-behavior
      builder-behavior
