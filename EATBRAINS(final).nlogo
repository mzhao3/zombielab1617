...
to zombify
ifelse infectionlength > incubation-period
  [ hatch-zombies 1 [
    rt 90
...
to developcure
  set cure-development cure-development + (count zombies / 10000)
end
...
; Moving Around
  ...
  [shambleback]
  [
    ifelse any? humans with [infected? != true and doctor = true] in-cone zombie-vision-radius zombie-vision-angle
    [
...
; Infection
let x random 100
if x < infection-chance
  [ ask humans-here [set infected? true] ]
]
end

to breaksh*t
  if pcolor > gray and pcolor <= white
...
to killdoctors
  ...
  [ if any? humans with [doctor = true] in-cone zombie-vision-radius zombie-vision-angle
    [ face min-one-of humans with [doctor = true] [distance myself] fd zombie-speed * 1.5 ]
  ]
end
...
## BREAKDOWN
 ...
 1.12.17 added doctor behavior
 1.13.17 added rotting rate and crumble
 1.15.17 added infection chance, infection length, and turning humans (zombify)
 11.16.17 wrote Info tab
Kevin Ly
Setup buildings and fixed setup turtles
setup patient0
added basic human and zombie behaviors
added patient0 behavior
added hunter-behavior and builder-behavior
1.12.17 added breaksh*t
1.16.17 fixed all behaviors
@#$#@#$#@
default
true
0
