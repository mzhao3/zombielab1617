...
humans-own [ stamina role builder hunter doctor nextshape pleb infected? safecolor infectionlength]
...
to assignroles
  set nextshape "person"
  if random 100 < 10
  [ set role builder
    set nextshape "builder"]

  if random 100 < 8
  [ set role doctor
    set nextshape "doctor"]

  if random 100 < 20
  [ set role hunter
    set nextshape "hunter" ]
; Maggie
end
...
if infected? [set infectionlength infectionlength + 1
  turnzombie]
...
to go
  ...
  [
      ...
      ask humans with [role = doctor ] [doctor-behavior]
