...
to setup
  ...
  set-default-shape zombies "zombie1"
  ; Creating Humans
  ...
  ; Creating Zombies
  ask n-of Starting_Zombies patches with [ pcolor = black ]
    [
      sprout-zombies 1
      [
...
to setupsafezonewhite
  let safezonex random (max-pxcor - 11)
  let safezoney random (2 * max-pycor - 11) - (max-pxcor)
  ask patches [
    if pxcor >= safezonex and pxcor <= safezonex + 11
    and pycor >= safezoney and pycor <= safezoney + 11
    [set pcolor white
      set safe? true] ]
end

to setupsafezoneblue
  let safezonex random (max-pycor - 11 ) - (max-pxcor)
  let safezoney random (2 * max-pycor - 11) - (max-pycor)
...
to zombie-behavior
  if viewablevision?
  [ask patches in-cone zombie_vision_radius zombie_vision_angle with [ not (safe? = true )]
    [set pcolor yellow ]
  ]

  ifelse any? patches in-cone 1 360 with [ (safe? = true) ] [
  ...
  [
    ...
    [
      face min-one-of humans [distance myself] fd .4]
    [shamble]
  ask humans-here
  [
    hatch-zombies 1
...
Polygon -7500403 true true 105 75 120 180 90 270 105 285 135 285 150 210 165 285 195 285 210 270 180 180 195 75
Rectangle -7500403 true true 127 64 172 79
Polygon -7500403 true true 195 75 285 60 285 90 180 105
Polygon -7500403 true true 255 120 165 120 165 150 240 150
